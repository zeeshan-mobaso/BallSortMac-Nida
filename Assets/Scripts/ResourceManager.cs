...
public partial class ResourceManager
{
    ...
    {
        _modeAndLevels.Clear();
        //Debug.LogError("_modeLvlAssets....." + _modeLvlAssets.Count);
    ...
    public static bool IsLevelLocked(GameMode mode, int no)
    {
        ...
        var completedLevel = PrefManager.GetInt($"{mode}_Level_Complete{no}");

        //Debug.LogError("IsLevelLocked....." + no);

        bool lvllocked = false;

        if(no > 1)
        {
            if (PrefManager.GetInt($"{mode}_Level_Complete{no - 1}") == 1)
            {
                lvllocked = false;
            }
            else
        ...
        //Debug.LogError("Completed Level....."+ completedLevel);
    ...
    //public static int GetCompletedLevel(GameMode mode , int no)
    //{
    //    //Debug.LogError($"Level No : {no}  completed  {PrefManager.GetInt($"{mode}_Level_Complete{no}")}");
    //    return PrefManager.GetInt($"{mode}_Level_Complete");
    //}
    public static bool GetCompletedLevel(GameMode mode, int no)
    {
        if (PrefManager.GetInt($"{mode}_Level_Complete{no}") == 1)
            return true;
        else
            return false;
        //return PrefManager.GetInt($"{mode}_Level_Complete{no}");
    }
    public static void CompleteLevel(GameMode mode, int lvl)
    {
        ...
        PrefManager.SetInt($"{mode}_Level_Complete{lvl}", 1);
